...
bool AndroidImgui::Init_Render(ANativeWindow *window, float width,
                               float height) {
  m_Window = window;
  m_Width = width;
  m_Height = height;
...
TextureInfo AndroidImgui::LoadTextureData(
...
TextureInfo AndroidImgui::LoadTextureFromFile(const char *filepath) {
  return LoadTextureData([filepath](BaseTexData *tex_data) {
    return stbi_load(filepath, &tex_data->Width, &tex_data->Height, nullptr,
                     tex_data->Channels);
  });
}
...
TextureInfo_gif AndroidImgui::LoadTextureFromMemory_gif(void *data, int len) {
  ...
  stbi_uc *gif_alldata = stbi_load_gif_from_memory(
      (const stbi_uc *)data, len, &delays, &w, &h, &frames, &n, 0);
  if (gif_alldata == NULL) {
    return textureInfo_gif;
  }
  textureInfo_gif.DS =
      (unsigned long long *)malloc(sizeof(unsigned long long) * frames);
