...
int main(int argc, char *argv[]) {
    // ★ v2.48: 禁用 stdout/stderr 缓冲, 确保日志实时输出 (诊断安卓16卡死问题)
    setvbuf(stdout, NULL, _IONBF, 0);
    setvbuf(stderr, NULL, _IONBF, 0);

    printf("\n\033[36m========== overlay v2.49 启动 ==========\033[0m\n");
    printf("\033[36m[*] 安卓版本: %d (SDK %d)\033[0m\n", getAndroidVersion(), getAndroidSDKLevel());
    fflush(stdout);
    ...
    // ★ v2.48: 综合修复安卓16触摸卡死 — 多重检测 + 非阻塞IO + poll超时
    // v2.47: 检查 Touch::Init 返回值, 失败时折叠UI避免全屏遮挡
    printf("\033[36m[*] 正在初始化触摸驱动...\033[0m\n"); fflush(stdout);
    bool touch_ok = Touch::Init(
            {static_cast<float>(::displayInfo.width), static_cast<float>(::displayInfo.height)},
            true);
    Touch::setOrientation(displayInfo.orientation);
    Touch::setOrientation(displayInfo.orientation);
    if (!touch_ok) {
        printf("\033[31m[!] Touch::Init 失败 (安卓%d), 触摸输入不可用, UI默认折叠\033[0m\n",
               getAndroidVersion());
        printf("\033[33m    音量+键可展开UI查看状态, 音量-键折叠\033[0m\n");
        MemuSwitch = false;  // 触摸失效时折叠UI, 用户可用音量键手动展开
    }
    ...
    std::thread([]{
        ...
        if (api_fetch_game_offsets()) {
            // 成功提示已在 api_fetch_game_offsets() 内部打印
        } else {
            printf("\033[33m[!] 服务端偏移获取失败, 使用本地回退值\033[0m\n");
        }
    }).detach();
