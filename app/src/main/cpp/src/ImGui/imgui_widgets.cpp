...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf,
  ...
  if (g.ActiveId == id) {
    ...
    if (apply_edit_back_to_user_buffer) {
      ...
      // priority over any underlying modification of the input buffer User
      // callback
  ...
  if (is_multiline)
    line_count = InputTextLineIndexBuild(
        ...
        (render_cursor && state && state->CursorFollow) ? INT_MAX
                                                        : line_visible_n1 + 1,
  ...
  if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
    if (g.Style.FrameBorderSize > 0.0f)
      ...
      window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg),
                                rounding); // Color buttons are often in need of
