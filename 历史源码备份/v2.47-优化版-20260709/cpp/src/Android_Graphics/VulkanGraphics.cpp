...
bool VulkanGraphics::Create() {
  ...
  ImGui_ImplVulkan_LoadFunctions(
      0,
      [](const char *function_name, void *handle) -> PFN_vkVoidFunction {
...
// Select Present Mode
    ...
    ImGui_ImplVulkanH_CreateOrResizeWindow(
        m_Instance, m_PhysicalDevice, m_Device, wd.get(), m_QueueFamily,
...
void VulkanGraphics::Render(ImDrawData *drawData) {
  ...
  {
    // 透明 默认已经是0了
    // memset(wd->ClearValue.color.float32, 0,
    // sizeof(wd->ClearValue.color.float32));
...
uint32_t VulkanGraphics::findMemoryType(uint32_t type_filter,
...
BaseTexData *VulkanGraphics::LoadTexture(BaseTexData *tex, void *pixel_data) {
  ...
  // Create the Vulkan image.
  {
    ...
    err = vkAllocateMemory(m_Device, &alloc_info, m_Allocator,
                           &tex_data->ImageMemory);
    check_vk_result(err);
    err =
        vkBindImageMemory(m_Device, tex_data->Image, tex_data->ImageMemory, 0);
    check_vk_result(err);
  }
  ...
  // Create Sampler
  {
    ...
    err = vkCreateSampler(m_Device, &sampler_info, m_Allocator,
                          &tex_data->Sampler);
    check_vk_result(err);
  }
  ...
  // Create Upload Buffer
  {
    ...
    err = vkCreateBuffer(m_Device, &buffer_info, m_Allocator,
    ...
    check_vk_result(err);
    err = vkBindBufferMemory(m_Device, tex_data->UploadBuffer,
  ...
  {
    void *map = NULL;
    err = vkMapMemory(m_Device, tex_data->UploadBufferMemory, 0, image_size, 0,
                      &map);
    check_vk_result(err);
