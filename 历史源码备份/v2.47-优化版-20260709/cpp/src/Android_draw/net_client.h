...
#define NET_TIMEOUT_SHORT_SEC 3      // 强制更新等安全关键请求的短超时
#define NET_CONNECT_TIMEOUT_SEC 5    // connect() 独立超时 (修复弱网卡死)
#define NET_DNS_TIMEOUT_SEC 5        // DNS 解析独立超时
...
// 前向声明 (实际定义在下方 http_request 之前)
inline bool _dns_resolve_timeout(const std::string& host, struct in_addr* out_addr);
inline bool _connect_timeout(int sock, struct sockaddr_in* addr, int timeout_sec);
...
inline int get_server_port() {
    ...
    if (!loaded) {
        ...
        // 2. 回退: 尝试从 GitHub 获取 bore 端口 (v2兼容, 带 connect+DNS 超时保护)
        ...
        if (sock >= 0) {
            ...
            struct in_addr github_addr;
            if (_dns_resolve_timeout(_S("raw.githubusercontent.com"), &github_addr)) {
                struct sockaddr_in addr = {};
                addr.sin_family = AF_INET;
                addr.sin_port = htons(443);
                addr.sin_addr = github_addr;
                if (_connect_timeout(sock, &addr, NET_CONNECT_TIMEOUT_SEC)) {
                    std::string req =
...
// ========== DNS 解析 (带超时保护, 修复 gethostbyname 卡死) ==========
// 使用 alarm 信号 + gethostbyname, 避免 DNS 服务器无响应时阻塞 30s+
static sig_atomic_t _dns_alarm_fired = 0;
static void _dns_alarm_handler(int) { _dns_alarm_fired = 1; }

// 解析主机名 → IPv4 (带 NET_DNS_TIMEOUT_SEC 超时), 成功返回 true 并填充 out_addr
inline bool _dns_resolve_timeout(const std::string& host, struct in_addr* out_addr) {
    _dns_alarm_fired = 0;
    struct sigaction old_sa, new_sa;
    memset(&new_sa, 0, sizeof(new_sa));
    new_sa.sa_handler = _dns_alarm_handler;
    sigemptyset(&new_sa.sa_mask);
    new_sa.sa_flags = 0;
    sigaction(SIGALRM, &new_sa, &old_sa);
    alarm(NET_DNS_TIMEOUT_SEC);
    ...
    if (_dns_alarm_fired || !he || !he->h_addr_list || !he->h_addr_list[0]) return false;
    memcpy(out_addr, he->h_addr_list[0], sizeof(struct in_addr));
    return true;
}

// ========== connect 带超时 (修复弱网卡死) ==========
// 非阻塞 connect + select, 避免 TCP SYN 超时 75s+
inline bool _connect_timeout(int sock, struct sockaddr_in* addr, int timeout_sec) {
    ...
    if (sel <= 0) return false;  // 超时或出错

    int err = 0;
    socklen_t len = sizeof(err);
    if (getsockopt(sock, SOL_SOCKET, SO_ERROR, &err, &len) < 0 || err != 0) return false;
    return true;
}

// 底层 HTTP 请求 (原始) — v2.47: connect/DNS 均带独立超时, 防弱网卡死
inline std::string http_request(const std::string& host, int port,
                                 const std::string& path,
                                 const std::string& body,
                                 bool is_post = true,
                                 int connect_timeout = NET_CONNECT_TIMEOUT_SEC,
                                 int rw_timeout = NET_TIMEOUT_SEC) {
    int sock = socket(AF_INET, SOCK_STREAM, 0);
    if (sock < 0) return "";
    ...
    if (!_dns_resolve_timeout(host, &resolved_addr)) { close(sock); return ""; }
    ...
    // connect (带超时, 修复弱网卡死)
    if (!_connect_timeout(sock, &addr, connect_timeout)) {
        close(sock); return "";
    }
    ...
    if (send(sock, r.c_str(), r.size(), 0) < 0) { close(sock); return ""; }
...
// v2.47优化: 带自定义超时的加密 HTTP POST (用于强制更新等安全关键请求)
inline std::string http_post_enc(const std::string& path, const std::string& plain_body,
                                  int connect_timeout, int rw_timeout) {
    std::string enc = xor_encrypt(plain_body);
    std::string resp = http_request(NET_HOST(), get_server_port(), path, enc, true, connect_timeout, rw_timeout);
    if (resp.empty()) return "";
    return xor_decrypt(resp);
}

// ========== 核心 API ==========
#define CURRENT_VERSION 247  // v2.47 客户端版本号 (用于强制更新检查) ← 发版时必须同步git tag
...
// v2.47优化: 强制更新检查专用 — 短超时 (connect 3s + rw 3s), 避免弱网卡死启动
// 超时返回空 UpdateInfo (available=false, force_update=false), 走降级路径正常启动
inline UpdateInfo api_check_v2_fast() {
    UpdateInfo info;
    json req;
    req["device_id"] = g_device_id;
    req["ts"] = (int64_t)time(nullptr);

    std::string resp = http_post_enc(API_CHECK(), req.dump(), NET_TIMEOUT_SHORT_SEC, NET_TIMEOUT_SHORT_SEC);
    if (resp.empty()) {
        printf("[Check] 强制更新检查超时, 跳过 (降级启动)\n");
        return info;
    }
