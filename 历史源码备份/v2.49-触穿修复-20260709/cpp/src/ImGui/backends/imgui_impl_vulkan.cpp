...
// please share them and your feedback at https://github.com/ocornut/imgui/ You
// can use unmodified imgui_impl_* files in your project. See examples/ folder
// for examples of using this. Prefer including the entire imgui/ repository
// into your project (either as a copy or as a submodule), and only build the
...
pc; out gl_PerVertex { vec4 gl_Position; }; layout(location = 0) out struct {
vec4 Color; vec2 UV; } Out; void main()
{
Out.Color = aColor;
Out.UV = aUV;
...
static VkPipeline ImGui_ImplVulkan_CreatePipeline(
    ...
    VkSampleCountFlagBits MSAASamples, uint32_t subpass,
  ...
  if (bd->VulkanInitInfo.UseDynamicRendering) {
    IM_ASSERT(pipeline_rendering_create_info &&
              "PipelineRenderingCreateInfo must not be nullptr when using "
              "dynamic rendering");
    IM_ASSERT(pipeline_rendering_create_info->sType ==
                  VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR &&
              "PipelineRenderingCreateInfo::sType must be "
              "VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR");
    IM_ASSERT(pipeline_rendering_create_info->pNext == nullptr &&
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
  ...
  if (v->RenderPass
      ...
      || (v->UseDynamicRendering &&
          v->PipelineRenderingCreateInfo.sType ==
              VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR)
#endif
  ) {
...
void ImGui_ImplVulkan_CreateMainPipeline(
    const ImGui_ImplVulkan_MainPipelineCreateInfo &info) {
  ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
  ImGui_ImplVulkan_InitInfo *v = &bd->VulkanInitInfo;
  if (bd->Pipeline) {
  ...
  bd->Pipeline = ImGui_ImplVulkan_CreatePipeline(
      v->Device, v->Allocator, v->PipelineCache, v->RenderPass, v->MSAASamples,
      v->Subpass, pipeline_rendering_create_info);
}
void ImGui_ImplVulkan_DestroyDeviceObjects() {
  ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
  ImGui_ImplVulkan_InitInfo *v = &bd->VulkanInitInfo;
