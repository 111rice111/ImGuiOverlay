...
struct Functionals {
  ...
  void (*LayerMetadata__setInt32)(void *thiz, uint32_t key,
  ...
  StrongPointer<void> (*SurfaceComposerClient__CreateSurface_and9)(
      void *thiz, void *name, uint32_t w, uint32_t h, int32_t format,
      uint32_t flags, void *parentHandle, int32_t windowType,
      int32_t ownerUid) = nullptr;
  ...
  Functionals(const SymbolMethod &symbolMethod) {
    ...
    static std::unordered_map<size_t, std::unordered_map<void **, const char *>>
        patchesTable = {
            ...
            },
            {
                ...
                {
                    {reinterpret_cast<void **>(
                         &SurfaceComposerClient__CreateSurface_and9),
                     "_ZN7android21SurfaceComposerClient13createSurfaceERKNS_"
    ...
    ResolveMethod(LayerMetadata, setInt32, libgui,
    ...
    ResolveMethod(SurfaceComposerClient, GetInternalDisplayToken, libgui,
                  "_ZN7android21SurfaceComposerClient23getInternalDisplayTokenE"
                  "v"); // 小于或者等于安卓13
    ...
    ResolveMethod(SurfaceComposerClient, GetDisplayInfo, libgui,
                  ...
                  "2spINS_7IBinderEEEPNS_11DisplayInfoE"); // 安卓10及以下
    ResolveMethod(
...
struct SurfaceComposerClient {
  ...
  SurfaceControl CreateSurface(const char *name, int32_t width, int32_t height,
    ...
    if (skipScrenshot && (Functionals::GetInstance().systemVersion == 10 ||
                          Functionals::GetInstance().systemVersion == 11)) {
    ...
    // ★ v2.49: 所有安卓12+ 设置 trusted overlay 实现 input 穿透
    // 根因: 通过 SurfaceComposerClient 创建的全屏 Surface 在安卓14+ 会拦截
    //       系统触摸事件 (没有 InputQueue 处理, 事件被丢弃), 导致整个系统
    //       触摸失效 — 不只是游戏, 连返回桌面/点击其他App都无效.
    //       UI 仍能响应是因为 evdev 旁路读取不依赖 InputFlinger.
    // 修复: setTrustedOverlay(true) 告诉 InputFlinger 此 overlay 不消费 input,
    //       触摸事件穿透到下层窗口 (游戏/桌面/其他App).
    // 安全性: 安卓15 等本就不拦截的版本设置后无副作用 (事件本就穿透).
    //         skipScrenshot 时也设置 (保留原有防截图用途).
    if (12 <= Functionals::GetInstance().systemVersion) {
  ...
  bool GetDisplayInfo(ui::DisplayState *displayInfo) {
    ...
    if (defaultDisplayToken.get() == nullptr) {
      if (9 >= Functionals::GetInstance().systemVersion) { // 小于或者等于安卓9
        ...
        if (14 >
            Functionals::GetInstance().systemVersion) { // 小于或者等于安卓13
          defaultDisplayToken =
...
public:
  ...
  static DisplayInfo GetDisplayInfo() {
    ...
    int32_t local_abs_x = (displayInfo.layerStackSpaceRect.width >
                                   displayInfo.layerStackSpaceRect.height
                               ? displayInfo.layerStackSpaceRect.width
                               : displayInfo.layerStackSpaceRect.height);
    int32_t local_abs_y = (displayInfo.layerStackSpaceRect.width <
                                   displayInfo.layerStackSpaceRect.height
                               ? displayInfo.layerStackSpaceRect.width
                               : displayInfo.layerStackSpaceRect.height);
