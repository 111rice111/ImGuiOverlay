...
// issues compiling/linking/running/loading fonts: please post in
...
// a database of cross-linked knowledge there. Library Version (Integer encoded
// as XYYZZ for use in #if preprocessor conditionals, e.g. '#if
...
enum ImGuiInputTextFlags_ {
  ...
  //   buffer: +~0.3 ms (Optimized) / +~1.0 ms (Debug build).
  //   The CPU cost is very roughly proportional to text length, so a 10 KB
  //   buffer should cost about ten times less.
  ImGuiInputTextFlags_WordWrap =
      1 << 24, // InputTextMultine(): word-wrap lines that are too long.
  // Obsolete names
...
enum ImGuiKey : int {
  ...
  //   backends tend to interfere and break that equivalence. The safer decision
...
enum ImGuiTableFlags_ {
  ...
  ImGuiTableFlags_NoClip =
      1
      << 20, // Disable clipping rectangle for every individual columns (reduce
...
struct ImGuiStyle {
  ...
  float FontScaleDpi;  // Additional global scale factor from viewport/monitor
                      // contents scale. When io.ConfigDpiScaleFonts is enabled,
                      // this is automatically overwritten when changing monitor
                      // DPI.
...
//   ImGui::IsKeyPressed(MYPLATFORM_KEY_SPACE)                  --> New (1.87+)
...
// keyboard state. The first 512 are now unused and should be kept zero. Legacy
// backend will write into KeyMap[] using ImGuiKey_ indices which are always
// >512. bool      KeysDown[ImGuiKey_COUNT];           // [LEGACY] Input:
// Keyboard keys that are pressed (ideally left in the "native" order your
// engine has access to keyboard keys, so you can use your own defines/enums for
// keys). This used to be [512] sized. It is now ImGuiKey_COUNT to allow legacy
// io.KeysDown[GetKeyIndex(...)] to work without an overflow. float
// NavInputs[ImGuiNavInput_COUNT];     // [LEGACY] Since 1.88, NavInputs[] was
// removed. Backends from 1.60 to 1.86 won't build. Feed gamepad inputs via
// io.AddKeyEvent() and ImGuiKey_GamepadXXX enums. void*     ImeWindowHandle; //
// [Obsoleted in 1.87] Set ImGuiViewport::PlatformHandleRaw instead. Set this to
...
// ImGuiListClipper(int items_count, float items_height = -1.0f) { memset(this,
// 0, sizeof(*this)); ItemsCount = -1; Begin(items_count, items_height); } //
// [removed in 1.79]
#endif
};
...
struct ImFontConfig {
  ...
  bool
      PixelSnapV; // true     // Align Scaled GlyphOffset.y to pixel boundaries.
  ImS8
...
// - This should not affect you as you can always use the latest value. But note
// that any precomputed UV coordinates are only valid for the current TexRef.
